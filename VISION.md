...
배포는 시작일 뿐이다. 지금은 배포하고 끝인데, 실제 운영에서는 다양한 액션이 필요하다.

**대상 컨트랙트:** 템플릿 컨트랙트(ERC20, LP 등)와 Generic Upload로 배포한 커스텀 컨트랙트 모두.
ABI가 있으면 write 함수를 자동으로 파싱해서 UI를 생성한다.

**우선순위 순서:**
1. **Proxy 업그레이드** — 새 Implementation 배포 + ERC1967 교체. 가장 중요한 운영 액션.
2. **긴급 정지** — pause/unpause. 사고 대응 시간을 최소화.
3. **권한 이전** — owner 변경. 팀 구성 변화에 대응.
4. **파라미터 변경** — 수수료율, 한도 등 운영 중 조정이 필요한 값.
...
| Phase 1 (완료) | 범용 배포 + 템플릿 라이브러리 | MVP 안정화 |
