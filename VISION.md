...
- `ContractActionPanel` — 함수 선택 → 파라미터 입력 (bool/uint/address/string/raw-hex/disabled) → 실행
...
**다음 단계 (Phase 2-B):**
- DEX 배포 시나리오 end-to-end: ERC20 2개 → LiquidityPool → `addLiquidity()` 액션 실행
- 액션 이력 조회 UI: `contract_actions` 데이터를 대시보드에서 표시
- CertiK Explorer 소스코드 검증 플로우 검토
...
| Phase 2-B | DEX 시나리오 검증 + 액션 이력 조회 UI | 🔜 다음 |
